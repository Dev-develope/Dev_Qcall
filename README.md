...
//               >
//                 {item.text}
//                 {item.hasChildren && <span><ChevronRight/></span>}
//               </button>
        ...
        <div className="flex flex-col py-6 px-5 space-y-4">
          {menuItems.map((item) => (
            <div key={item.id} className="w-full text-left">
              ...
              <button
                className={`w-full text-sm py-3 flex justify-between items-center transition-colors duration-200 ${
              ...
              {/* {item.hasChildren && (
                ...
                >
                  <ul className="bg-[#121212] text-white py-2 border-t border-gray-700 hover:text-[#00a7e6]">
                    {item.subMenu.map((sub) => (
                      <li key={sub.url} className="py-2 px-4">
                        <Link
                          href={sub.url}
