  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-6">
      <div className="flex flex-col items-center mb-8">
        ...
        <p className="text-sm text-[#00a7e6] text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
        {ecosystemData.map((item, index) => (
          <div
            key={index}
            className="border border-gray-200 rounded-lg p-6 flex flex-col justify-between h-full"
          >
            <div>
              <h2 className="font-bold text-lg mb-1">{item.title}</h2>
              <p className="text-xs text-gray-600 mb-4">{item.description}</p>

              <h3 className="text-sm font-light text-xl mb-2">Use Cases</h3>
              <div className="space-y-4">
                {item.useCases.map((useCase, i) => (
                  <div className="flex items-start gap-2" key={i}>
                    <div className="text-[#00a7e6] flex-shrink-0">•</div>
