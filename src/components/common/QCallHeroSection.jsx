  ...
  return (
    <div
      className={`relative w-full overflow-hidden ${
        img ? "lg:[50rem] sm:h-full" : "h-[45rem]"
      }`}
    >
      {img ? (
        ...
        <div className="relative flex flex-col items-center justify-center min-h-screen px-4 py-16 text-center">
          ...
          <div className="grid md:grid-cols-3 gap-6 w-full max-w-4xl px-4 text-white">
            {driveMoreRevenueData.map((card, index) => (
              ...
              >
                <span className="text-xs mb-2 block">{card.category}</span>
          ...
          <CommonButton
            title="Create your AI agent in 30 seconds"
            className="mt-12 bg-[#00a7e6] text-white font-light py-3 px-8 rounded-full transition-all duration-300 transform hover:scale-105 hover:shadow-lg"
